...
class ReleaseActionServer{
	...
	public:
		...
		ReleaseActionServer(std::string name):
		as_(nh_, name, boost::bind(&ReleaseActionServer::executeCB, this, _1), false)
		, action_name_(name){
			as_.start();
		...
		void executeCB(const arm_control_pkg::StageGoalConstPtr &goal){
			ROS_INFO("RELEASE ACTION EXECUTING");
			...
			//if stage start is true execute releaseing of arm if it completes succesfully return true as result
...
{
  ros::init(argc, argv, "release_as");

  ReleaseActionServer demo("release_as");
  ros::spin();
